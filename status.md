...
- `Release`
  ...
  - `v1.7.5, master` means a single change is in both `v1.7.5` release branch and the `master` branch. Such translations occupy only one row in the table. If the translation was made on `master` and ported to release branch in a separate commit (or vice versa), it must be two rows in the table.
- `EN base`
